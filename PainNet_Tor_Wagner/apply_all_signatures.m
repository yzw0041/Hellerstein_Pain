...
% ::
%
%     SIG = apply_all_signatures(DATA_OBJ, ['similarity_metric', sim_metric, 'image_scaling', imgscaling, 'conditionnames', conditionnames)
%
...
% -------------------------------------------------------------------------
[signature_obj, signaturenames, imgnames] = load_image_set('npsplus');
...
for j = 1:length(signaturenames)
    ...
    for i = 1:k
        ...
        varname = [signaturenames{j} '_' similarity_metric '_' image_scaling];
            
        sigtable{i}.(varname) = myresponse;
        
        sigtable{i}.Properties.VariableDescriptions{j} = sprintf('%s signature response. Similarity = %s, Data scaling = %s', signaturenames{j}, similarity_metric, image_scaling);
        
    end
    
    SIG.(signaturenames{j}) = conditiontable;

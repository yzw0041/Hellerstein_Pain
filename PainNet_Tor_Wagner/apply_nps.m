function [nps_values, image_names, data_objects, npspos_exp_by_region, npsneg_exp_by_region] = apply_nps(input_images, varargin)
%
...
%
% [nps_values, image_names, data_objects, npspos_exp_by_region, npsneg_exp_by_region] = apply_nps(input_images)
%
%
...
% image_names
% data_objects
% npspos_exp_by_region  local pattern expression in subregions defined by nps pos mask
% npsneg_exp_by_region  local pattern expression in subregions defined by nps neg mask
...
% [nps_values, ~, ~, npspos_exp_by_region, npsneg_exp_by_region] = apply_nps(data_objects);
%
% See also:
% apply_mask.m
...
if nargout > 3
    % for individual subregions
    % Prep individual region masks
    npspos = resample_space(fmri_data(npspos), data_objects{1});
    npsneg = resample_space(fmri_data(npsneg), data_objects{1});
end

for i = 1:length(data_objects)
    
    if verbose
        nps_values{i} = apply_mask(data_objects{i}, mask, 'pattern_expression', 'cosine_similarity'); %, 'ignore_missing');
    ...
    if nargout > 3
        ...
        clpos = extract_roi_averages(data_objects{i}, npspos, 'pattern_expression', 'contiguous_regions', 'nonorm');
        clneg = extract_roi_averages(data_objects{i}, npsneg, 'pattern_expression', 'contiguous_regions', 'nonorm');

        % save data from individual regions
        
        npspos_exp_by_region{i} = cat(2, clpos.dat);
        npsneg_exp_by_region{i} = cat(2, clneg.dat);
        
    end
    %         % re-sort data from individual regions
    %
    %     for j = 1:length(clpos)
    %         npspos_exp_by_region{j}(:, i) = clpos(j).dat;
    %     end
    %
    %     for j = 1:length(clneg)
    %         npsneg_exp_by_region{j}(:, i) = clneg(j).dat;
    %     end
    
end

% PRINT TABLE OF NPS VALUES
% -------------------------------------------------------------------------
if verbose || dotables
    for i = 1:length(data_objects)
